...
(setq TeX-auto-save t)        ; these slow down instantiation of latex, but are worth it
(setq TeX-parse-self t)
(setq TeX-auto-untabify t)    ; convert tabs to spaces on save
(setq-default TeX-master nil) ; make AUCTeX aware of multi-file doc structure "\include"
;(setq text-mode-hook  '(lambda () (flyspell-mode 1))) ; highlight mispelled word
(setq flyspell-mode t)        ; highlight mispelled words
;;(add-hook 'latex-mode-hook 'set-word-wrap)  ; enable soft wrap for latex mode

; key bindings
;(local-set-key [(space)] 'smart-space) ; bind the 'smart-space' function to 'space' key
(local-set-key [(meta a)] 'define-mode-abbrev) ; define abbrevs on the fly with Alt-a
(local-set-key [(meta space)] 'dabbrev-expand) ; expand dynamic abbreviations with Alt-space
...
(defun rjp-latex-binomial ()  ; insert \binom{}{} and move the cursor back 3 chars
  (interactive)         
  (insert "\\binom{}{}")  
  (backward-char 3)      ; move back 3 chars, that is, inside the first pair of braces
  )
